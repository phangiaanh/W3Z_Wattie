'''
code adapted from Splice https://github.com/omerbt/Splice/blob/master/models/extractor.py
'''
...
class VitExtractor(nn.Module):
    ...
    def __init__(self, model_name, frozen,):
        super(VitExtractor, self).__init__()  # Initialize the superclass
        if 'dinov2' in model_name:
            self.model = torch.hub.load('facebookresearch/dinov2', model_name, pretrained=True)#.to(self.device)
        else:
            self.model = torch.hub.load('facebookresearch/dino:main', model_name, pretrained=True)#.to(self.device)
        if frozen:
           ...
           # self.model.eval()
            ...
            for name, param in self.model.named_parameters():
                if not name.startswith("blocks.9") and not name.startswith("blocks.10") and not name.startswith("blocks.11"):
    ...
    def get_attn_feature_from_input(self, input_img): # List([B, N, D, D])
        self._register_hooks()
        self.model(input_img)
    ...
    def get_patch_size(self):
        if "8" in self.model_name:
            return 8
        elif hasattr(self.model, "patch_size"):
    ...
    def get_queries_from_qkv(self, qkv, input_img_shape):
        ...
        batch = qkv.shape[0]
        q = qkv.reshape(batch, patch_num, 3, head_num, embedding_dim // head_num).permute(0, 2, 3, 1, 4)[:,0]
        return q
    ...
    def get_keys_from_qkv(self, qkv, input_img_shape):
        ...
        batch = qkv.shape[0]
        k = qkv.reshape(batch, patch_num, 3, head_num, embedding_dim // head_num).permute(0, 2, 3, 1, 4)[:,1]
        return k
    ...
    def get_values_from_qkv(self, qkv, input_img_shape):
        ...
        batch = qkv.shape[0]
        v = qkv.reshape(batch, patch_num, 3, head_num, embedding_dim // head_num).permute(0, 2, 3, 1, 4)[:,2]
        return v
    ...
    def get_keys_self_sim_from_input(self, input_img, layer_num = 11):
        ''' calculate difference in self-similarity of the keys; current only for batch = 1'''
    ...
    def get_cls_token(self, input_img, layer_num = 11):
        cls_token = self.get_feature_from_input(input_img)[layer_num][:, 0, :]
        return cls_token
